...
contains
  ...
  real(dp) function p_U(U)
    ...
    type(Euler1D_U_State), intent(in) :: U
    p_U = gm1*(U%E - 0.5*U%du*U%du/U%rho)
    return
  end function p_U
  ...
  integer function unphysical_properties_U(U) result(ierr)
    type(Euler1D_U_State), intent(in) :: U
    if((U%rho.lt.0.).or.(U%rho.ne.U%rho)) then
     ierr = 1
    else if(p_U(U).lt.0.) then
     ierr = 1
    else if((U%E.lt.0.).or.(U%E.ne.U%E)) then
     ierr = 1
    else
     ierr = 0
    end if
  ...
  subroutine dFdU_U(U, dFdU)
    ...
    dFdU(1,2) = 1.
    dFdU(1,3) = 0.
    dFdU(2,1) = 0.5*(g-3.)*v*v
    dFdU(2,2) = (3.-g)*v
    dFdU(2,3) = gm1
    dFdU(3,1) = gm1*v*v*v - g*v*U%E/U%rho
    dFdU(3,2) = g*U%E/U%rho - 0.5*3.*gm1*v*v
    dFdU(3,3) = g*v
...
contains
  ...
  real(dp) function E_W(W)
    use gasConstants, only: gm1i
    type(Euler1D_W_State), intent(in) :: W
    E_W = gm1i*W%p + 0.5*W%rho*W%u*W%u
  end function E_W
  ...
  real(dp) function Esp_W(W)
    use gasConstants, only: gm1i
    type(Euler1D_W_State), intent(in) :: W
    Esp_W = gm1i*W%p/W%rho + 0.5*W%u*W%u
  end function Esp_W
  ...
  real(dp) function H_W(W)
    use gasConstants, only: g, gm1i
    type(Euler1D_W_State), intent(in) :: W
    H_W = g*gm1i*W%p + 0.5*W%rho*W%u*W%u
  end function H_W
  ...
  real(dp) function Hsp_W(W)
    use gasConstants, only: g, gm1i
    type(Euler1D_W_State), intent(in) :: W
    Hsp_W = g*gm1i*W%p/W%rho + 0.5*W%u*W%u
  end function Hsp_W
  ...
  integer function unphysical_properties_W(W) result(ierr)
    type(Euler1D_W_State), intent(in) :: W
    if((W%rho.lt.0.).or.(W%rho.ne.W%rho)) then
     ierr = 1
    else if((W%p.lt.0.).or.(W%p.ne.W%p)) then
     ierr = 1
    else if(E_W(W).lt.0.) then
     ierr = 1
    else
     ierr = 0
    end if
  ...
  subroutine dFdU_W(W, dFdU)
    ...
    dFdU(1,2) = 1.
    dFdU(1,3) = 0.
    dFdU(2,1) = 0.5*(g-3.)*W%u*W%u
    dFdU(2,2) = (3.-g)*W%u
    dFdU(2,3) = gm1
    dFdU(3,1) = gm1*W%u*W%u*W%u - g*W%u*E/W%rho
    dFdU(3,2) = g*E/W%rho - 0.5*3.*gm1*W%u*W%u
    dFdU(3,3) = g*W%u
...
contains
  ...
  real(dp) function H_Q(Q)
    ...
    real(dp)                    :: rho
    rho = rho_Q(Q)
    H_Q = g*gm1i*Q%p + 0.5*rho*Q%u*Q%u
  end function H_Q
  ...
  real(dp) function Hsp_Q(Q)
    use gasConstants, only: g, gm1i, R
    type(Euler1D_Q_State), intent(in) :: Q
    Hsp_Q = R*g*gm1i*Q%T + 0.5*Q%u*Q%u
  end function Hsp_Q
  ...
  integer function unphysical_properties_Q(Q) result(ierr)
    type(Euler1D_Q_State), intent(in) :: Q
    if((Q%p.lt.0.).or.(Q%p.ne.Q%p)) then
     ierr = 1
    else if((Q%T.lt.0.).or.(Q%T.ne.Q%T)) then
     ierr = 1
    else if(E_Q(Q).lt.0.) then
     ierr = 1
    else
     ierr = 0
    end if
  ...
  subroutine dFdU_Q(Q, dFdU)
    ...
    dFdU(1,2) = 1.
    dFdU(1,3) = 0.
    dFdU(2,1) = 0.5*(g-3.)*Q%u*Q%u
    dFdU(2,2) = (3.-g)*Q%u
    dFdU(2,3) = gm1
    dFdU(3,1) = gm1*Q%u*Q%u*Q%u - g*Q%u*E/rho
    dFdU(3,2) = g*E/rho - 0.5*3.*gm1*Q%u*Q%u
    dFdU(3,3) = g*Q%u
